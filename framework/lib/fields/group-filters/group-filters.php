...
function group_filters_settings_field( $name, $settings, $value ) {
	...
	ob_start();
	?>
	<select name="<?php echo $helper_instance->esc_attr( $input_name ); ?>" id="smile_<?php echo $helper_instance->esc_attr( $input_name ); ?>" class="select2-group_filters-dropdown form-control smile-input <?php echo $helper_instance->esc_attr( 'smile-' . $type . ' ' . $input_name . ' ' . $type . ' ' . $class ); ?>" multiple="multiple" style="width:260px;">
		...
		foreach ( $selected_values as $key => $sel_value ) {
			...
			if ( false !== strpos( $sel_value, 'post-' ) ) {
				...
				echo '<option value="post-' . $helper_instance->esc_attr( $post_id ) . '" selected="selected" >' . $helper_instance->esc_attr( $post_title ) . '</option>';
			}
			...
			if ( false !== strpos( $sel_value, 'tax-' ) ) {
				...
				echo '<option value="tax-' . $helper_instance->esc_attr( $tax_id ) . '" selected="selected" >' . $helper_instance->esc_attr( $term->name ) . ' - ' . $helper_instance->esc_attr( $term_taxonomy ) . '</option>';
			}
			...
			foreach ( $spacial_pages as $page => $title ) {
				...
				if ( $selected ) {
					echo "<option selected='selected' value='special-" . $helper_instance->esc_attr( $page ) . "' >" . $helper_instance->esc_attr( $title ) . '</option>';
				}
			}
		}
