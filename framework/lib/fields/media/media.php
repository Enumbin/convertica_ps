...
function media_settings_field( $name, $settings, $value, $default_value = null ) {
	...
	$btn_label   = ( '' !== $value ) ? $helper_instance->__( 'Change Image', 'smile' ) : $helper_instance->__( 'Select Image', 'smile' );
	...
	$img     = ( '' == $value ) ? '<p class="description">' . $helper_instance->__( 'No Image Selected', 'smile' ) . '</p>' : '<img src="' . $image_src . '"/>';
	...
	if ( '' === $default_value ) {
		$output .= '<button style="' . $rmv_btn . '" id="remove_' . $input_name . '_' . $uid . '" ' . $display . ' class="button button-secondary smile-remove-media form-control smile-input smile-' . $type . '">' . $helper_instance->__( 'Remove', 'smile' ) . '</button>';
	}
	$output    .= '<button style="' . $dflt_btn . '" data-default="' . $default_value . '" id="default_' . $input_name . '_' . $uid . '" ' . $display . ' class="button button-secondary smile-default-media form-control smile-input smile-' . $type . '">' . $helper_instance->__( 'Default', 'smile' ) . '</button>';
	...
	$output    .= '<strong><label for="smile_' . $input_name . '_size">' . $helper_instance->__( 'Select Size', 'smile' ) . '</label></strong>';
	$output    .= '<p>';

	$image_url = $helper_instance->wp_get_attachment_url( $img_arr[0] ); // Just the file name.
...
function cp_get_all_image_sizes() {
	...
	foreach ( $default_image_sizes as $size ) {
		$image_sizes[ $size ]['width']  = intval( $helper_instance->convertica_get_option( "{$size}_size_w" ) );
		$image_sizes[ $size ]['height'] = intval( $helper_instance->convertica_get_option( "{$size}_size_h" ) );
		$image_sizes[ $size ]['crop']   = $helper_instance->convertica_get_option( "{$size}_crop" ) ? $helper_instance->convertica_get_option( "{$size}_crop" ) : false;
	}
