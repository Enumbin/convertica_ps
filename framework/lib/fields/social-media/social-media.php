...
$helper_instance->add_action( 'admin_enqueue_scripts', 'social_media_box_scripts' );
/**
	...
	if ( isset( $_REQUEST['cp_admin_page_nonce'] ) && wp_verify_nonce( $_REQUEST['cp_admin_page_nonce'], 'cp_admin_page' ) ) {
		...
		if ( false !== $cp_page || isset( $_GET['view'] ) ) {
			if ( false !== $cp_page && isset( $data['cp-dev-mode'] ) && '1' === $data['cp-dev-mode'] && isset( $_GET['style-view'] ) && ( 'edit' === $_GET['style-view'] || 'variant' === $_GET['style-view'] ) ) {
				wp_enqueue_style( 'convert-plus-media', SMILE_FRAMEWORK_URI . '/lib/fields/social-media/social-media.css', array(), CP_VERSION );
				wp_enqueue_script( 'convert-plus-media', SMILE_FRAMEWORK_URI . '/lib/fields/social-media/social-media.js', array( 'jquery', 'convert-plus-swal-js' ), '1.0.0', true );
			}
		}
	}
}
...
function render_multi_social_media( $uniq, $value ) {
	...
	$fb_access_token                = isset( $_value_array['fb-access-token'] ) ? sanitize_text_field( $_value_array['fb-access-token'] ) : '';
	...
	$facebook_token_css   = 'style="display:none"';
	if ( 'Pinterest' === $current_input_type_val ) {
		$pintrest_img_url_css = 'style="display:block"';
	}
	if ( 'Facebook' === $current_input_type_val ) {
		$facebook_token_css = 'style="display:block"';
	}
	...
	$output           .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="Select social network from given list." style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="The network name will display along with icon. It\'s visibility depends on \'Layout\' selection & \'Display Network Names\' setting in \'Advance\' section." style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	// add Facebook Access Token field.
	$output .= '<div class="social-media-field cp-net-name show-fb-token" ' . $facebook_token_css . '>';
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="This kind of access token is needed to modify and read app settings. It can also be used to publish Open Graph actions. It is generated using a pre-agreed secret between the app and Facebook and is then used during calls that change app-wide settings. You obtain an app access token via a server-to-server call." style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	$output .= '<label>' . __( 'Facebook Access Token', 'smile' ) . '</label>';
	$output .= '<input type="text" class="cp_sm_input" id="cp-fb-access-token-' . $uniq . '" name="fb-access-token" value="' . $fb_access_token . '"/>';
	$output .= '</div>';
	...
	$output                  .= '<div class="social-media-field">';
	$output                  .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="Select action for social icon click." style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="Enter the social profile URL where user redirect after click." style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	$output .= '</div>';
			...
			// switch for share.
	$output .= '<div class="social-media-field " >';
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="' . $share_url_desc . '" style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	$output .= '</div>';
			...
			// share url.
	$output .= '<div class="social-media-field">';
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="' . $custom_url_desc . '" style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
	...
	$output .= '<span class="cp-tooltip-icon has-tip" data-position="right" title="Enter the image ULR to share" style="cursor: help;
	float: right;"><i class="dashicons dashicons-editor-help"></i></span>';
...
function social_media_settings_field( $name, $settings, $value ) {
	...
	$output .= '<input type="hidden" id="cp_social_media_nonce" value="' . esc_attr( $social_media_nonce ) . '">';
...
function repeat_social_media_callback() {
	...
	check_ajax_referer( 'cp_social_media_nonce', 'security' );

	$uniq = sanitize_text_field( $_POST['id'] );
