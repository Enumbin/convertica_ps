...
if ( isset( $_GET['theme'] ) && 'floating_social_bar' === $_GET['theme'] ) {
	$option_array = array(
		$helper_instance->__( 'Normal', 'smile' ) => 'normal',
		$helper_instance->__( 'Border', 'smile' ) => 'border',
		$helper_instance->__( 'Flip', 'smile' )   => 'flip',
		$helper_instance->__( 'Grow', 'smile' )   => 'grow',
	);
} else {
	$option_array = array(
		$helper_instance->__( 'Normal', 'smile' ) => 'normal',
		$helper_instance->__( 'Slide', 'smile' )  => 'slide',
	);
}
...
$cp_social = array(
	...
	array(
		...
		'opts'         => array(
			...
			'options'    => array(
				$helper_instance->__( 'cp-icon-style-left', 'smile' )      => CP_BASE_URL . '/modules/assets/images/icon_with_left.png',
				$helper_instance->__( 'cp-icon-style-right', 'smile' )     => CP_BASE_URL . '/modules/assets/images/icon_with_right.png',
				$helper_instance->__( 'cp-icon-style-simple', 'smile' )    => CP_BASE_URL . '/modules/assets/images/simple_icon.png',
				$helper_instance->__( 'cp-icon-style-rectangle', 'smile' ) => CP_BASE_URL . '/modules/assets/images/icon_with_square.png',
			),
			'width'      => '125px',
			'imagetitle' => array(
				$helper_instance->__( 'title-0', 'smile' ) => 'Icon At Left',
				$helper_instance->__( 'title-1', 'smile' ) => 'Icon At Right',
				$helper_instance->__( 'title-2', 'smile' ) => 'Icon At Left Without Background',
				$helper_instance->__( 'title-3', 'smile' ) => 'Icon At Top',
			),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Number of Columns', 'smile' ),
			'value'       => 'auto',
			'options'     => array(
				$helper_instance->__( 'Auto Width', 'smile' ) => 'auto',
				$helper_instance->__( '1', 'smile' )          => '1',
				$helper_instance->__( '2', 'smile' )          => '2',
				$helper_instance->__( '3', 'smile' )          => '3',
				$helper_instance->__( '4', 'smile' )          => '4',
				$helper_instance->__( '5', 'smile' )          => '5',
				$helper_instance->__( '6', 'smile' )          => '6',
			),
			'description' => $helper_instance->__( 'Select grid to display social icons', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( ' Icon Container Alignment', 'smile' ),
			'value'       => 'auto',
			'options'     => array(
			...
			'description' => $helper_instance->__( 'Select alignment for icon container', 'smile' ),
		),
		'dependency'   => array(
			'name'     => 'cp_social_icon_column',
			'operator' => '==',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Remove Icon Spacing', 'smile' ),
			'value'       => false,
			'on'          => $helper_instance->__( 'YES', 'smile' ),
			'off'         => $helper_instance->__( 'NO', 'smile' ),
			'description' => $helper_instance->__( 'Remove gap / spacing between two social icons', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'        => $helper_instance->__( 'Icon Container Border Radius', 'smile' ),
			'value'        => 5,
			'min'          => 0,
			'max'          => 50,
			'step'         => 1,
			...
			'css_preview'  => true,
			'description'  => $helper_instance->__( 'Apply border radius to icon container.', 'smile' ),
		),
		'dependency'   => array(
			'name'     => 'cp_social_icon_style',
			'operator' => '!==',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Icon Shape', 'smile' ),
			'value'       => 'Normal',
			'options'     => array(
				$helper_instance->__( 'Normal', 'smile' ) => 'normal',
				$helper_instance->__( 'Square', 'smile' ) => 'square',
				$helper_instance->__( 'Circle', 'smile' ) => 'circle',
				$helper_instance->__( 'Custom', 'smile' ) => 'border_radius',
			),
			'description' => $helper_instance->__( 'Provide shape to your icon.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'        => $helper_instance->__( 'Icon Border Radius', 'smile' ),
			'value'        => 5,
			'min'          => 0,
			'max'          => 50,
			'step'         => 1,
			...
			'css_preview'  => true,
			'description'  => $helper_instance->__( 'Apply border radius to actual icon.', 'smile' ),
		),
		'dependency'   => array(
			'name'     => 'cp_social_icon_shape',
			'operator' => '==',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Icon Effect', 'smile' ),
			'value'       => 'gradient',
			'options'     => array(
				$helper_instance->__( 'Flat', 'smile' )    => 'flat',
				$helper_instance->__( '3D', 'smile' )      => '3D',
				$helper_instance->__( 'Overlay', 'smile' ) => 'gradient',
			),
			'description' => $helper_instance->__( 'Style your icon container with nice effects.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Icon Hover Effect', 'smile' ),
			'value'       => 'Slide',
			'options'     => $option_array,
			'description' => $helper_instance->__( 'Apply slide / normal hover effect to icon.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Use Custom Colors', 'smile' ),
			'value'       => false,
			'on'          => $helper_instance->__( 'YES', 'smile' ),
			'off'         => $helper_instance->__( 'NO', 'smile' ),
			'description' => $helper_instance->__( 'Style your icons with custom colors.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'        => $helper_instance->__( 'Icon Color', 'smile' ),
	...
	array(
		...
		'opts'         => array(
			'title' => $helper_instance->__( 'Text Color', 'smile' ),
			'value' => 'rgb(255, 255, 255)',
		),
		'dependency'   => array(
			'name'     => 'cp_social_enable_icon_color',
	...
	array(
		...
		'opts'         => array(
			'title'        => $helper_instance->__( 'Icon Hover Color', 'smile' ),
			'value'        => 'rgb(255, 255, 255)',
			'css_property' => 'color',
	...
	array(
		...
		'opts'         => array(
			'title' => $helper_instance->__( 'Text Hover Color', 'smile' ),
			'value' => 'rgb(255, 255, 255)',
		),
		'dependency'   => array(
			'name'     => 'cp_social_enable_icon_color',
	...
	array(
		...
		'opts'         => array(
			'title'        => $helper_instance->__( 'Background Hover Color', 'smile' ),
			'value'        => '#0e72b4',
			'css_property' => 'background',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Display Network Names', 'smile' ),
			'value'       => true,
			'on'          => $helper_instance->__( 'YES', 'smile' ),
			'off'         => $helper_instance->__( 'NO', 'smile' ),
			'description' => $helper_instance->__( 'Show / hide social network name.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Display Share Counts', 'smile' ),
			'value'       => false,
			'on'          => $helper_instance->__( 'YES', 'smile' ),
			'off'         => $helper_instance->__( 'NO', 'smile' ),
			'description' => $helper_instance->__( 'Show / hide share counts.', 'smile' ),
		),
		'panel'        => 'Social Networks',
		'section'      => 'Design',
		'section_icon' => 'connects-icon-disc',
	...
	array(
		...
		'opts'         => array(
			'title'       => $helper_instance->__( 'Minimum Count Display', 'smile' ),
			'value'       => 50,
			'min'         => 0,
			'max'         => 1000,
			'step'        => 1,
			'description' => $helper_instance->__( 'Display minimum share count number until actual count increases. Actual count will display only on front end.', 'smile' ),
		),
		'dependency'   => array(
			'name'     => 'cp_social_share_count',
			'operator' => '==',
...
$helper_instance->add_filter( 'cp_get_social', 'cp_get_social_init' );
				...
				if ( '1' === $cp_social_share_count ) {
					if ( '' !== $social_min_count ) {
						...
						if ( '' == $current_page ) {
							...
							if ( 'facebook' === $input_type ) {
								$access_token = isset( $value['fb-access-token'] ) ? $value['fb-access-token'] : '';
								$current_page = $current_page . '&access_token=' . $access_token;
							}
...
$helper_instance->add_filter( 'cp_social_css', 'cp_social_css_init' );
/**
 * Generate & Append CSS.
 *
 * @param  array $a setting array.
	...
	function cp_get_share_count( $type, $current_page ) {
		...
		if ( in_array( $count_name, $share_array ) ) {
			if ( 'facebook' === $count_name ) {
				$count_url = 'https://graph.facebook.com/?id=' . $current_page . '&fields=engagement';
		...
		if ( 'twitter' === $count_name ) {
			$count_url = 'http://opensharecount.com/count.json?url=' . $current_page;
		}
		...
		if ( 'facebook' === $count_name ) {
			$shares = isset( $count_data['engagement']['share_count'] ) ? $count_data['engagement']['share_count'] : '';
			if ( '' !== $shares ) {
				return $shares;
			}
		} elseif ( 'twitter' === $count_name ) {
